                                ...
                                <div class="form-group col-lg-12 col-md-12 col-sm-12 col-xs-12">
                                    ...
                                    <div class="col-md-11 form-option-line">
                                        <div class="webuploader-box js-upload-file" upload-type="files"
                                             input-name="demo_webuploader_files_1">
                                            <div class="uploader-list ui-files-sortable">
                                                @if($webuploader_files1!=='')
                                                    @foreach(explode(',',$webuploader_files1) as $v)
                                                        <li class="list-group-item file-item upload-state-done"
                                                            ...
                                                            <input type="hidden" name="demo_webuploader_files_1[]"
                                                                   value="{{$v}}">
                                            ...
                                            <span class="form-control-static form-option-line help-line form-option-webuploader-line">多文件上传1的提示信息</span>
                                        </div>
                                    </div>
                                </div>
                                <div class="form-group col-lg-12 col-md-12 col-sm-12 col-xs-12">
                                    ...
                                    <div class="col-md-11 form-option-line">
                                        <div class="webuploader-box js-upload-file" upload-type="files"
                                             input-name="demo_webuploader_files_2">
                                            <div class="uploader-list ui-files-sortable">
                                                @if($webuploader_files2!=='')
                                                    @foreach(explode(',',$webuploader_files2) as $v)
                                                        <li class="list-group-item file-item upload-state-done"
                                                            style="word-wrap: break-word;">
                                                            <span class="pull-right file-state"></span>
                                                            <i class="fa fa-file"></i>{{$v}}&nbsp;&nbsp;
                                                            ...
                                                            <input type="hidden" name="demo_webuploader_files_2[]"
                                                                   value="{{$v}}">
                                                            <i class="fa fa-fw fa-arrows move-file"
                                                               style="display: none;"></i>
                                                        </li>
    ...
    <script>
        $(function () {
            $(document).on('click', '#demo-submit', function () {
                $('#demo-form').find('.form-validate-msg').remove();//清空该表多的验证错误信息
                var data = $('#demo-form').serialize();//表多数据
                Dolphin.loading('提交中...');
                $.ajax({
                    type: 'POST',
                    url: '{{action('Admin\System\DemoController@webuploaderFilesSave')}}',
                    dataType: 'JSON',
                    data: data,
                    ...
                    error: function (xhr, status, error) {
                        ...
                        if (xhr.status == 422) { //数据指定错误，错误码固定为422
                            ...
                            $.each(response.errors, function (k, v) {
                                ...
                                $('#create-' + k).append(validate_tips); // 页面表多项下方提示，错误验证信息
                            });
