                                        ...
                                        <div class="row">
                                            <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                                                ...
                                                @foreach($treeNodes as $node)
                                                    @if($node['level']<4)
                                                        ...
                                                        <div class="small-input-checkbox" style="margin-left: {{$marginLeft}}px;">
                                                            <label style="@if($node['level']==1)font-weight: bold;@endif" title="@if($node['level']==1)模块@else{{$node['level']}}级节点@endif" class="css-input css-checkbox css-checkbox-primary css-checkbox-sm css-checkbox-rounded">
                                                                <input level="{{$node['level']}}" type="checkbox" name="system_node_ids[]" value="{{$node['id']}}" @if(in_array($node['id'],\App\Servers\ArrServer::ids($systemRole->systemNodes))) checked @endif>
                                                                <span></span> {{$node['name']}}
                                                                @if($node['level']==1)
                                                    ...
                                                    @else
                                                        @php
                                                            $checked=in_array($node['id'],\App\Servers\ArrServer::ids($systemRole->systemNodes))?'checked':'';
                                ...
                                <div class="form-group col-lg-12 col-md-12 col-sm-12 col-xs-12" id="create-status">
                                    ...
                                    <div class="col-md-11 form-option-line">
                                        <label class="css-input switch switch-sm switch-primary switch-rounded " title="启用/禁用">
                                            <input type="checkbox" name="status" value="1" @if($systemRole->status==1) checked @endif><span></span>
                                        </label>
    ...
    <script>
        $(function () {
            $(document).on('click', '#create-submit', function () {
                ...
                $.ajax({
                    type: 'PUT',
                    url: '{{action('Admin\System\RoleController@update',['id'=>$systemRole->id])}}',
                    dataType: 'JSON',
                    data: data,
